...
function setRole(role) {
  ...
  if (dashboardNode) {
    // The dashboard view is already mounted; we need to re‑render it.
    // We'll use a custom event to trigger re‑render from inside the view.
...
function fillActivityFeed(container, students, fees, salaries) {
  // Combine events from all collections with a timestamp and description
  const activities = [];
...
function fillUpcomingEvents(container) {
  // Currently no events collection – show "No upcoming events"
  container.innerHTML = "";
  container.appendChild(el("div", { class: "state" }, [
    el("div", { html: ICON.inbox }),
    el("div", { class: "state-sub", text: "No upcoming events scheduled." })
  ]));
  // Future: if an events collection exists, subscribe and render here.
}
