...
export async function updateStudent(id, payload, photoFile) {
  ...
  await update(dbRef(db, `${PATH.students}/${id}`), {
    ...payload,
...
export async function deleteStudent(id) {
  await remove(dbRef(db, `${PATH.students}/${id}`));
}

export async function getStudent(id) {
  return getById(PATH.students, id);
}

export function subscribeStudents(cb) {
  return subscribeCollection(PATH.students, cb);
}
...
export async function updateTeacher(id, payload, photoFile) {
  ...
  await update(dbRef(db, `${PATH.teachers}/${id}`), {
    ...payload,
    photoUrl: photoUrl ?? null,
    updatedAt: nowMs()
  });
...
export async function deleteTeacher(id) {
  await remove(dbRef(db, `${PATH.teachers}/${id}`));
}

export async function getTeacher(id) {
  return getById(PATH.teachers, id);
}

export function subscribeTeachers(cb) {
  return subscribeCollection(PATH.teachers, cb);
}
...
export async function deleteFee(id) {
  await remove(dbRef(db, `${PATH.fees}/${id}`));
}

export async function getFee(id) {
  return getById(PATH.fees, id);
}

export function subscribeFees(cb) {
  return subscribeCollection(PATH.fees, cb);
}
...
export async function deleteSalary(id) {
  await remove(dbRef(db, `${PATH.salaries}/${id}`));
}

export async function getSalary(id) {
  return getById(PATH.salaries, id);
}

export function subscribeSalaries(cb) {
  return subscribeCollection(PATH.salaries, cb);
}
